...
var InstallationsRouter = exports.InstallationsRouter = function (_ClassesRouter) {
  ...
  _createClass(InstallationsRouter, [{
    ...
    value: function handleFind(req) {
      ...
      console.log('APDEBUG >> 2 body >> ' + JSON.stringify(req.body));
      console.log('APDEBUG >> 2 config >> ' + JSON.stringify(req.config));
      console.log('APDEBUG >> 2 auth >> ' + JSON.stringify(req.auth));

      return _rest2.default.find(req.config, req.auth, '_Installation', req.body.where, options).then(function (response) {
